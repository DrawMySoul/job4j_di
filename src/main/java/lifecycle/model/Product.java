...
public class Product implements BeanNameAware, BeanFactoryAware, ApplicationContextAware, InitializingBean, DisposableBean {
    ...
    public Product(String name, double price) {
        System.out.printf("1. Вызов конструктора %s%n", getClass().getSimpleName());
        this.name = name;
        this.price = price;
    }
    ...
    public void annotInitMethod() {
        System.out.printf("4. @PostConstruct выполняются действия при инициализации бина '%s'%n", beanName);
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        System.out.printf("4. InitializingBean: выполняются действия при инициализации бина '%s'%n", beanName);
    }

    public void initMethod() {
        System.out.printf("4. @Bean, свойство initMethod выполняются действия при инициализации бина '%s'%n", beanName);
    }
    ...
    public void destroy() throws Exception {
        System.out.printf("6. Disposable: выполняются действия при уничтожении бина '%s'%n", beanName);
    }
